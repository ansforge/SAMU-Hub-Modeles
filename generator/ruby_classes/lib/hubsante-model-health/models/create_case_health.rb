...
module Hubsante::Model::Health
  class CreateCaseHealth
    # A valoriser avec l'identifiant de l'affaire/dossier partagé entre tous les intervenants, valorisé comme suit : {pays}.{domaine}.{organisation}.{senderCaseId}. Cet identifiant est généré une seule fois par le système du partenaire récepteur de la primo-demande de secours (créateur du dossier). Il doit pouvoir être généré de façon décentralisée et ne présenter aucune ambiguïté. Il doit être unique dans l'ensemble des systèmes : le numéro de dossier fourni par celui qui génère l'identifiant partagé doit donc être un numéro unique dans son système.
    ...
    # Attribut qui permet de transférer la prise en charge d'un dossier à un autre CRAA - après accord verbal de ce dernier.
    attr_accessor :owner
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Hubsante::Model::Health::CreateCaseHealth` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Hubsante::Model::Health::CreateCaseHealth`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [case_id, sender_case_id, creation, perimeter, intervention_type, qualification, location, initial_alert, owner, patient, medical_note, new_alert, additional_information].hash
    end
    ...
    def self._deserialize(type, value)
      ...
      when /\AHash<(?<k_type>.+?), (?<v_type>.+)>\z/
        ...
        klass = Hubsante::Model::Health.const_get(type)
