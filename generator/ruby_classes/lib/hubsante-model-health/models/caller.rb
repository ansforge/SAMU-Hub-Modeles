  ...
  class Caller
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Hubsante::Model::Health::Caller` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Hubsante::Model::Health::Caller`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('String', ["SUJET", "FAMILLE", "TIERS", "POMPIER", "AMBULANC", "SECOUR", "MED", "MEDSOS", "MRL", "EFFML", "SANTE", "INF", "AIDESOIN", "SF", "AIDEDOM", "FDO-MILI", "ADM-TUTL", "VIP", "OBJCONNC", "AUTRE", "INCONNU"])
