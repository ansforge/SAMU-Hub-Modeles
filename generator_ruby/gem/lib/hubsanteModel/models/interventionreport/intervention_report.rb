  ...
  class InterventionReport
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Interventionreport::InterventionReport` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Interventionreport::InterventionReport`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'redactor')
        self.redactor = attributes[:'redactor']
      else
        self.redactor = nil
      end
    ...
    def list_invalid_properties
      ...
      if @redactor.nil?
        invalid_properties.push('invalid value for "redactor", redactor cannot be nil.')
      end
    ...
    def hash
      [case_id, report_id, redactor, creation, patient, evaluation].hash
    end
