...
module Emsi
  class Position
    ...
    # Optionnel
    attr_accessor :height_role

    # Optionnel
    attr_accessor :coordsys
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Emsi::Position` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Emsi::Position`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'height_role')
        self.height_role = attributes[:'height_role']
      end

      if attributes.key?(:'coordsys')
        self.coordsys = attributes[:'coordsys']
      end
    ...
    def hash
      [loc_id, name, type, height_role, coordsys, coord, address].hash
    end
