...
def test_convert_cisu_invalid_direction(client):
    ...
    # Load base envelope
    envelope = TestHelper.load_json_file(
...
def test_convert_from_cisu(client, rs_target_version):
    edxl_json = TestHelper.create_edxl_json_from_schema(
        TestConstants.EDXL_FIRE_TO_HEALTH_ENVELOPE_PATH, TestConstants.RC_EDA_TAG
    )
    response = client.post(
        "/convert",
        json={
...
def test_convert_to_cisu(client, rs_source_version):
    edxl_json = TestHelper.create_edxl_json_from_schema(
        TestConstants.EDXL_HEALTH_TO_FIRE_ENVELOPE_PATH, TestConstants.RS_EDA_TAG
    )
    response = client.post(
        "/convert",
        json={
